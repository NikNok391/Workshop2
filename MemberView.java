...
public class MemberView {
    ...
    private void createMember() {
        ...
        ms.addBoat(name, Integer.parseInt(length), Integer.parseInt(type));
    ...
    private void list() {
        ...
        if(listStyle.equals("C") || listStyle.equals("c")){
            List<Member> currentMem = ms.listMember();
            System.out.println("Name \tID \tNumber of boats:");
            for (int i = 0; i < currentMem.size(); i++) {
                System.out.println(currentMem.get(i).getName() + "\t" + currentMem.get(i).getId() + "\t" + currentMem.get(i).getNumberBoats());
            }
        } else if(listStyle.equals("D") || listStyle.equals("d")){
            System.out.println("Name \tID \tNumber of boats:");
            for (int i = 0; i < ms.listMember().size(); i++) {
                System.out.println(ms.listMember().get(i).getName() + "\t" + ms.listMember().get(i).getId() + "\t" + ms.listMember().get(i).getNumberBoats());
                 System.out.println("\tNum \tLength\tType");
                    for (int j = 0; j < ms.listBoat(ms.listMember().get(i).getName()).size(); j++) {
                        System.out.println("\t" + ms.listBoat(ms.listMember().get(i).getName()).get(j).getNum() + "\t" + ms.listBoat(ms.listMember().get(i).getName()).get(j).getLength() + "\t" + ms.listBoat(ms.listMember().get(i).getName()).get(j).getType());
                    }
            }
        }
    }
        ...
        if (ms.findMember(name) != -1) {
            ...
            if (change.equals("P") || change.equals("p")) {
                ...
                if (boatsUpdate.equals("U") || boatsUpdate.equals("u")) {
                    ...
                    System.out.println("Num \tLength\tType");
                    for (int i = 0; i < ms.listBoat(name).size(); i++) {
                        System.out.println(ms.listBoat(name).get(i).getNum() + "\t" + ms.listBoat(name).get(i).getLength() + "\t" + ms.listBoat(name).get(i).getType());
                    }
